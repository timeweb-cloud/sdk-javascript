...
class Person {
     ...
     * @param id {Number} Уникальный идентификатор администратора домена.
     * @param type {module:model/Person.TypeEnum} Тип администратора: `person` — физическое лицо, `org` — организация, `ip` — индивидуальный предприниматель.
     * @param name {String} Имя администратора (ФИО или название организации).
     * @param nameEng {String} Имя администратора в латинской транслитерации.
     * @param email {String} Адрес электронной почты администратора.
     * @param phone {String} Контактный телефон администратора.
     * @param postcode {String} Почтовый индекс.
     * @param mailingAddress {String} Почтовый адрес.
     * @param countryCode {String} Код страны администратора.
     * @param isResident {Boolean} Это логическое значение, которое показывает, является ли администратор резидентом РФ.
     * @param isBlank {Boolean} Это логическое значение, которое показывает, заполнены ли данные администратора не полностью.
     * @param isClosed {Boolean} Это логическое значение, которое показывает, закрыт ли администратор.
     */
    constructor(id, type, name, nameEng, email, phone, postcode, mailingAddress, countryCode, isResident, isBlank, isClosed) { 
        
        Person.initialize(this, id, type, name, nameEng, email, phone, postcode, mailingAddress, countryCode, isResident, isBlank, isClosed);
    }
     ...
     */
    static initialize(obj, id, type, name, nameEng, email, phone, postcode, mailingAddress, countryCode, isResident, isBlank, isClosed) { 
        obj['id'] = id;
        obj['type'] = type;
        obj['name'] = name;
        obj['name_eng'] = nameEng;
        obj['email'] = email;
        obj['phone'] = phone;
    ...
    static constructFromObject(data, obj) {
        if (data) {
            ...
            if (data.hasOwnProperty('name_eng')) {
                obj['name_eng'] = ApiClient.convertToType(data['name_eng'], 'String');
            }
            ...
            if (data.hasOwnProperty('is_blank')) {
                obj['is_blank'] = ApiClient.convertToType(data['is_blank'], 'Boolean');
            }
    ...
    static validateJSON(data) {
        ...
        // ensure the json data is a string
        if (data['name_eng'] && !(typeof data['name_eng'] === 'string' || data['name_eng'] instanceof String)) {
            throw new Error("Expected the field `name_eng` to be a primitive type in the JSON string but got " + data['name_eng']);
        }
        // ensure the json data is a string
...
Person.RequiredProperties = ["id", "type", "name", "name_eng", "email", "phone", "postcode", "mailing_address", "country_code", "is_resident", "is_blank", "is_closed"];
 ...
 * @member {module:model/Person.TypeEnum} type
 */
Person.prototype['type'] = undefined;
 ...
 * @member {String} name_eng
 */
Person.prototype['name_eng'] = undefined;
